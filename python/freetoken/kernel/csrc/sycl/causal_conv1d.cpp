...
template <typename scalar_t>
void launch_q4_0_matvec_tiled(const torch::Tensor &x,
  ...
  queue.parallel_for(
      ...
      [=](sycl::nd_item<1> item) {
        ...
        for (int64_t index = item.get_local_id(0); index < in_features;
          ...
          const uint8_t *packed =
              weight_ptr + (row * blocks_per_row + block) * kBlockBytes;
          const uint8_t quant_byte = packed[2 + in_block % 16];
          const int quant = in_block < 16 ? quant_byte & 0x0F : quant_byte >> 4;
          const float d =
              static_cast<float>(*reinterpret_cast<const sycl::half *>(packed));
          const float weight = d * static_cast<float>(quant - 8);
...
torch::Tensor q4_0_matvec(torch::Tensor x, torch::Tensor qweight) {
  TORCH_CHECK(x.device().is_xpu(), "x must be an XPU tensor");
  ...
  TORCH_CHECK(x.size(1) % 32 == 0, "Q4_0 input width must be divisible by 32");
  TORCH_CHECK(qweight.size(1) == x.size(1) / 32 * 18,
              "qweight has invalid Q4_0 row geometry");
  auto output = torch::empty({x.size(0), qweight.size(0)}, x.options());
  if (output.numel() == 0) {
    return output;
  }
  if (x.scalar_type() == torch::kFloat32) {
    launch_q4_0_matvec_tiled<float>(x, qweight, output);
  } else if (x.scalar_type() == torch::kBFloat16) {
    launch_q4_0_matvec_tiled<sycl::ext::oneapi::bfloat16>(x, qweight, output);
  } else {
    TORCH_CHECK(false, "q4_0_matvec supports float32 and bfloat16 inputs");
  }
  return output;
}

template <typename scalar_t>
void launch_q4_1_matvec_tiled(const torch::Tensor &x,
                              const torch::Tensor &qweight,
                              torch::Tensor &output) {
  constexpr int64_t kBlockSize = 32;
  constexpr int64_t kBlockBytes = 20;
  constexpr int64_t kWorkgroupSize = 256;
  constexpr int64_t kTokenTile = 4;
  const int64_t batch = x.size(0);
  const int64_t in_features = x.size(1);
  ...
  queue.parallel_for(
      ...
      [=](sycl::nd_item<1> item) {
        ...
        for (int64_t index = item.get_local_id(0); index < in_features;
          ...
          const uint8_t *packed =
              weight_ptr + (row * blocks_per_row + block) * kBlockBytes;
          const uint8_t quant_byte = packed[4 + in_block % 16];
          const int quant = in_block < 16 ? quant_byte & 0x0F : quant_byte >> 4;
          const float d =
              static_cast<float>(*reinterpret_cast<const sycl::half *>(packed));
          const float m = static_cast<float>(
              *reinterpret_cast<const sycl::half *>(packed + 2));
          const float weight = d * static_cast<float>(quant) + m;
          for (int64_t tile = 0; tile < kTokenTile; ++tile) {
            const int64_t token = token_base + tile;
            if (token < batch) {
              partial[tile] +=
...
torch::Tensor q4_1_matvec(torch::Tensor x, torch::Tensor qweight) {
  TORCH_CHECK(x.device().is_xpu(), "x must be an XPU tensor");
  TORCH_CHECK(qweight.device() == x.device(),
              "qweight must be on the same XPU device as x");
  TORCH_CHECK(x.dim() == 2 && qweight.dim() == 2,
  ...
  TORCH_CHECK(qweight.scalar_type() == torch::kUInt8,
              "qweight must use uint8 storage");
  TORCH_CHECK(x.size(1) % 32 == 0, "Q4_1 input width must be divisible by 32");
  TORCH_CHECK(qweight.size(1) == x.size(1) / 32 * 20,
              "qweight has invalid Q4_1 row geometry");
  auto output = torch::empty({x.size(0), qweight.size(0)}, x.options());
  if (output.numel() == 0) {
    return output;
  }
  if (x.scalar_type() == torch::kFloat32) {
    launch_q4_1_matvec_tiled<float>(x, qweight, output);
  } else if (x.scalar_type() == torch::kBFloat16) {
    launch_q4_1_matvec_tiled<sycl::ext::oneapi::bfloat16>(x, qweight, output);
  } else {
    TORCH_CHECK(false, "q4_1_matvec supports float32 and bfloat16 inputs");
  }
  return output;
}

template <typename scalar_t>
void launch_iq4_nl_matvec_tiled(const torch::Tensor &x,
                                const torch::Tensor &qweight,
                                torch::Tensor &output) {
  constexpr int64_t kBlockSize = 32;
  constexpr int64_t kBlockBytes = 18;
  ...
  queue.parallel_for(
      ...
      [=](sycl::nd_item<1> item) {
        ...
        for (int64_t index = item.get_local_id(0); index < in_features;
          ...
          const float d =
              static_cast<float>(*reinterpret_cast<const sycl::half *>(packed));
          const float weight = d * codebook[quant];
          for (int64_t tile = 0; tile < kTokenTile; ++tile) {
            const int64_t token = token_base + tile;
            if (token < batch) {
              partial[tile] +=
...
torch::Tensor iq4_nl_matvec(torch::Tensor x, torch::Tensor qweight) {
  TORCH_CHECK(x.device().is_xpu(), "x must be an XPU tensor");
  TORCH_CHECK(qweight.device() == x.device(),
              "qweight must be on the same XPU device as x");
  TORCH_CHECK(x.dim() == 2 && qweight.dim() == 2,
  ...
  TORCH_CHECK(x.size(1) % 32 == 0,
              "IQ4_NL input width must be divisible by 32");
  TORCH_CHECK(qweight.size(1) == x.size(1) / 32 * 18,
              "qweight has invalid IQ4_NL row geometry");
  auto output = torch::empty({x.size(0), qweight.size(0)}, x.options());
  if (output.numel() == 0) {
    return output;
  }
  if (x.scalar_type() == torch::kFloat32) {
    launch_iq4_nl_matvec_tiled<float>(x, qweight, output);
  } else if (x.scalar_type() == torch::kBFloat16) {
    launch_iq4_nl_matvec_tiled<sycl::ext::oneapi::bfloat16>(x, qweight, output);
  } else {
    TORCH_CHECK(false, "iq4_nl_matvec supports float32 and bfloat16 inputs");
  }
  return output;
}

template <typename scalar_t>
void launch_q5_0_matvec_tiled(const torch::Tensor &x,
                              const torch::Tensor &qweight,
                              torch::Tensor &output) {
  constexpr int64_t kBlockSize = 32;
  constexpr int64_t kBlockBytes = 22;
  constexpr int64_t kWorkgroupSize = 256;
  constexpr int64_t kTokenTile = 4;
  const int64_t batch = x.size(0);
  const int64_t in_features = x.size(1);
  ...
  queue.parallel_for(
      ...
      [=](sycl::nd_item<1> item) {
        ...
        for (int64_t index = item.get_local_id(0); index < in_features;
          ...
          const uint8_t *packed =
              weight_ptr + (row * blocks_per_row + block) * kBlockBytes;
          const uint8_t quant_byte = packed[6 + in_block % 16];
          const int low = in_block < 16 ? quant_byte & 0x0F : quant_byte >> 4;
          const int high = (packed[2 + in_block / 8] >> (in_block % 8)) & 0x01;
          const float d =
              static_cast<float>(*reinterpret_cast<const sycl::half *>(packed));
          const float weight = d * static_cast<float>((high << 4 | low) - 16);
          for (int64_t tile = 0; tile < kTokenTile; ++tile) {
            const int64_t token = token_base + tile;
            if (token < batch) {
              partial[tile] +=
...
torch::Tensor q5_0_matvec(torch::Tensor x, torch::Tensor qweight) {
  TORCH_CHECK(x.device().is_xpu(), "x must be an XPU tensor");
  TORCH_CHECK(qweight.device() == x.device(),
              "qweight must be on the same XPU device as x");
  TORCH_CHECK(x.dim() == 2 && qweight.dim() == 2,
  ...
  TORCH_CHECK(qweight.scalar_type() == torch::kUInt8,
              "qweight must use uint8 storage");
  TORCH_CHECK(x.size(1) % 32 == 0, "Q5_0 input width must be divisible by 32");
  TORCH_CHECK(qweight.size(1) == x.size(1) / 32 * 22,
              "qweight has invalid Q5_0 row geometry");
  auto output = torch::empty({x.size(0), qweight.size(0)}, x.options());
  if (output.numel() == 0) {
    return output;
  }
  if (x.scalar_type() == torch::kFloat32) {
    launch_q5_0_matvec_tiled<float>(x, qweight, output);
  } else if (x.scalar_type() == torch::kBFloat16) {
    launch_q5_0_matvec_tiled<sycl::ext::oneapi::bfloat16>(x, qweight, output);
  } else {
    TORCH_CHECK(false, "q5_0_matvec supports float32 and bfloat16 inputs");
  }
  return output;
}

template <typename scalar_t>
void launch_q5_1_matvec_tiled(const torch::Tensor &x,
                              const torch::Tensor &qweight,
                              torch::Tensor &output) {
  constexpr int64_t kBlockSize = 32;
  constexpr int64_t kBlockBytes = 24;
  constexpr int64_t kWorkgroupSize = 256;
  constexpr int64_t kTokenTile = 4;
  const int64_t batch = x.size(0);
  const int64_t in_features = x.size(1);
  ...
  queue.parallel_for(
      ...
      [=](sycl::nd_item<1> item) {
        ...
        for (int64_t index = item.get_local_id(0); index < in_features;
          ...
          const uint8_t *packed =
              weight_ptr + (row * blocks_per_row + block) * kBlockBytes;
          const uint8_t quant_byte = packed[8 + in_block % 16];
          const int low = in_block < 16 ? quant_byte & 0x0F : quant_byte >> 4;
          const int high = (packed[4 + in_block / 8] >> (in_block % 8)) & 0x01;
          const int quant = (high << 4) | low;
          const float d =
              static_cast<float>(*reinterpret_cast<const sycl::half *>(packed));
          const float m = static_cast<float>(
              *reinterpret_cast<const sycl::half *>(packed + 2));
...
torch::Tensor q5_1_matvec(torch::Tensor x, torch::Tensor qweight) {
  TORCH_CHECK(x.device().is_xpu(), "x must be an XPU tensor");
  TORCH_CHECK(qweight.device() == x.device(),
              "qweight must be on the same XPU device as x");
  TORCH_CHECK(x.dim() == 2 && qweight.dim() == 2,
  ...
  TORCH_CHECK(qweight.scalar_type() == torch::kUInt8,
              "qweight must use uint8 storage");
  TORCH_CHECK(x.size(1) % 32 == 0, "Q5_1 input width must be divisible by 32");
  TORCH_CHECK(qweight.size(1) == x.size(1) / 32 * 24,
              "qweight has invalid Q5_1 row geometry");
  auto output = torch::empty({x.size(0), qweight.size(0)}, x.options());
  if (output.numel() == 0) {
    return output;
  }
  if (x.scalar_type() == torch::kFloat32) {
    launch_q5_1_matvec_tiled<float>(x, qweight, output);
  } else if (x.scalar_type() == torch::kBFloat16) {
    launch_q5_1_matvec_tiled<sycl::ext::oneapi::bfloat16>(x, qweight, output);
  } else {
    TORCH_CHECK(false, "q5_1_matvec supports float32 and bfloat16 inputs");
  }
  return output;
}

template <typename scalar_t>
void launch_q8_0_matvec_tiled(const torch::Tensor &x,
                              const torch::Tensor &qweight,
                              torch::Tensor &output) {
  constexpr int64_t kBlockSize = 32;
  constexpr int64_t kBlockBytes = 34;
  ...
  queue.parallel_for(
      ...
      [=](sycl::nd_item<1> item) {
        ...
        for (int64_t block = item.get_local_id(0); block < blocks_per_row;
          ...
          const auto *quants = reinterpret_cast<const int8_t *>(packed + 2);
          for (int64_t element = 0; element < kBlockSize; ++element) {
            const int64_t index = block * kBlockSize + element;
            const float weight = static_cast<float>(quants[element]) * scale;
            for (int64_t tile = 0; tile < kTokenTile; ++tile) {
              const int64_t token = token_base + tile;
              if (token < batch) {
                partial[tile] +=
...
torch::Tensor q8_0_matvec(torch::Tensor x, torch::Tensor qweight) {
  ...
  TORCH_CHECK(qweight.scalar_type() == torch::kUInt8,
              "qweight must use uint8 storage");
  TORCH_CHECK(x.size(1) % 32 == 0, "Q8_0 input width must be divisible by 32");
  TORCH_CHECK(qweight.size(1) == x.size(1) / 32 * 34,
              "qweight has invalid Q8_0 row geometry");
  auto output = torch::empty({x.size(0), qweight.size(0)}, x.options());
  if (x.scalar_type() == torch::kFloat32) {
    if (x.size(0) >= 4) {
      launch_q8_0_matvec_tiled<float>(x, qweight, output);
    } else {
      launch_q8_0_matvec<float>(x, qweight, output);
    }
  } else if (x.scalar_type() == torch::kBFloat16) {
    if (x.size(0) >= 4) {
      launch_q8_0_matvec_tiled<sycl::ext::oneapi::bfloat16>(x, qweight, output);
    } else {
      ...
      [=](sycl::nd_item<1> item) {
        ...
        for (int64_t index = item.get_local_id(0); index < in_features;
          ...
          const uint8_t quant_byte =
              packed[16 + group64 * 32 + (in_block % 32)];
          const int quant =
              in_block % 64 < 32 ? quant_byte & 0x0F : quant_byte >> 4;
...
template <typename scalar_t>
void launch_q4_k_matvec_tiled(const torch::Tensor &x,
                              const torch::Tensor &qweight,
                              torch::Tensor &output) {
  constexpr int64_t kBlockSize = 256;
  constexpr int64_t kBlockBytes = 144;
...
torch::Tensor q4_k_matvec(torch::Tensor x, torch::Tensor qweight) {
  ...
  if (x.scalar_type() == torch::kFloat32) {
    if (x.size(0) >= 4) {
      launch_q4_k_matvec_tiled<float>(x, qweight, output);
    } else {
      launch_q4_k_matvec<float>(x, qweight, output);
    }
  } else if (x.scalar_type() == torch::kBFloat16) {
    if (x.size(0) >= 4) {
      launch_q4_k_matvec_tiled<sycl::ext::oneapi::bfloat16>(x, qweight, output);
    } else {
      ...
      [=](sycl::nd_item<1> item) {
        ...
        for (int64_t index = item.get_local_id(0); index < in_features;
          ...
          const int64_t quant_offset =
              (scale_index / 8) * 32 + (group_in_half % 2) * 16 + in_block % 16;
...
template <typename scalar_t>
void launch_q2_k_matvec_tiled(const torch::Tensor &x,
                              const torch::Tensor &qweight,
                              torch::Tensor &output) {
  constexpr int64_t kBlockSize = 256;
  constexpr int64_t kBlockBytes = 84;
...
torch::Tensor q2_k_matvec(torch::Tensor x, torch::Tensor qweight) {
  ...
  if (x.scalar_type() == torch::kFloat32) {
    if (x.size(0) >= 4) {
      launch_q2_k_matvec_tiled<float>(x, qweight, output);
    } else {
      launch_q2_k_matvec<float>(x, qweight, output);
    }
  } else if (x.scalar_type() == torch::kBFloat16) {
    if (x.size(0) >= 4) {
      launch_q2_k_matvec_tiled<sycl::ext::oneapi::bfloat16>(x, qweight, output);
    } else {
...
template <typename scalar_t>
void launch_pq2_0_matvec(const torch::Tensor &x, const torch::Tensor &qweight,
                         torch::Tensor &output) {
  ...
  queue.parallel_for(
      ...
      [=](sycl::nd_item<1> item) {
        ...
        for (int64_t index = item.get_local_id(0); index < in_features;
          ...
          const uint8_t *packed =
              weight_ptr + (row * blocks_per_row + block) * kBlockBytes;
          const int shift = static_cast<int>((in_block % 4) * 2);
          const int quant = (packed[2 + in_block / 4] >> shift) & 0x03;
          const float scale =
              static_cast<float>(*reinterpret_cast<const sycl::half *>(packed));
          const float weight = static_cast<float>(quant - 1) * scale;
          partial +=
              static_cast<float>(x_ptr[token * in_features + index]) * weight;
        }
        const float sum = sycl::reduce_over_group(item.get_group(), partial,
                                                  sycl::plus<float>());
        if (item.get_local_id(0) == 0) {
...
template <typename scalar_t>
void launch_pq2_0_matvec_tiled(const torch::Tensor &x,
                               const torch::Tensor &qweight,
                               torch::Tensor &output) {
  constexpr int64_t kBlockSize = 128;
  constexpr int64_t kBlockBytes = 34;
...
torch::Tensor pq2_0_matvec(torch::Tensor x, torch::Tensor qweight) {
  TORCH_CHECK(x.device().is_xpu(), "x must be an XPU tensor");
  TORCH_CHECK(qweight.device() == x.device(),
              "qweight must be on the same XPU device as x");
  TORCH_CHECK(x.dim() == 2 && qweight.dim() == 2,
  ...
  TORCH_CHECK(qweight.scalar_type() == torch::kUInt8,
              "qweight must use uint8 storage");
  TORCH_CHECK(x.size(1) % 128 == 0,
              "PQ2_0 input width must be divisible by 128");
  TORCH_CHECK(qweight.size(1) == x.size(1) / 128 * 34,
              "qweight has invalid PQ2_0 row geometry");
  auto output = torch::empty({x.size(0), qweight.size(0)}, x.options());
  if (x.scalar_type() == torch::kFloat32) {
    if (x.size(0) >= 4) {
      launch_pq2_0_matvec_tiled<float>(x, qweight, output);
    } else {
      launch_pq2_0_matvec<float>(x, qweight, output);
    }
  } else if (x.scalar_type() == torch::kBFloat16) {
    if (x.size(0) >= 4) {
      launch_pq2_0_matvec_tiled<sycl::ext::oneapi::bfloat16>(x, qweight,
                                                             output);
    } else {
      launch_pq2_0_matvec<sycl::ext::oneapi::bfloat16>(x, qweight, output);
    }
  } else {
    TORCH_CHECK(false, "pq2_0_matvec supports float32 and bfloat16 inputs");
  }
  return output;
}
  ...
  queue.parallel_for(
      ...
      [=](sycl::nd_item<1> item) {
        ...
        for (int64_t index = item.get_local_id(0); index < in_features;
          ...
          const int scale_low =
              (packed[96 + (scale_word % 2) * 4 + scale_byte] >>
               (scale_word >= 2 ? 4 : 0)) &
              0x0F;
          const int scale_high =
              (packed[104 + scale_byte] >> (scale_word * 2)) & 0x03;
...
template <typename scalar_t>
void launch_q3_k_matvec_tiled(const torch::Tensor &x,
                              const torch::Tensor &qweight,
                              torch::Tensor &output) {
  constexpr int64_t kBlockSize = 256;
  constexpr int64_t kBlockBytes = 110;
...
torch::Tensor q3_k_matvec(torch::Tensor x, torch::Tensor qweight) {
  ...
  if (x.scalar_type() == torch::kFloat32) {
    if (x.size(0) >= 4) {
      launch_q3_k_matvec_tiled<float>(x, qweight, output);
    } else {
      launch_q3_k_matvec<float>(x, qweight, output);
    }
  } else if (x.scalar_type() == torch::kBFloat16) {
    if (x.size(0) >= 4) {
      launch_q3_k_matvec_tiled<sycl::ext::oneapi::bfloat16>(x, qweight, output);
    } else {
  ...
  queue.parallel_for(
      ...
      [=](sycl::nd_item<1> item) {
        ...
        for (int64_t index = item.get_local_id(0); index < in_features;
          ...
          const uint8_t quant_byte =
              packed[48 + group64 * 32 + (in_block % 32)];
...
template <typename scalar_t>
void launch_q5_k_matvec_tiled(const torch::Tensor &x,
                              const torch::Tensor &qweight,
                              torch::Tensor &output) {
  constexpr int64_t kBlockSize = 256;
  constexpr int64_t kBlockBytes = 176;
...
torch::Tensor q5_k_matvec(torch::Tensor x, torch::Tensor qweight) {
  ...
  if (x.scalar_type() == torch::kFloat32) {
    if (x.size(0) >= 4) {
      launch_q5_k_matvec_tiled<float>(x, qweight, output);
    } else {
      launch_q5_k_matvec<float>(x, qweight, output);
    }
  } else if (x.scalar_type() == torch::kBFloat16) {
    if (x.size(0) >= 4) {
      launch_q5_k_matvec_tiled<sycl::ext::oneapi::bfloat16>(x, qweight, output);
    } else {
...
template <typename scalar_t>
void launch_q6_k_matvec_tiled(const torch::Tensor &x,
                              const torch::Tensor &qweight,
                              torch::Tensor &output) {
  constexpr int64_t kBlockSize = 256;
  constexpr int64_t kBlockBytes = 210;
  constexpr int64_t kWorkgroupSize = 256;
  constexpr int64_t kTokenTile = 4;
  const int64_t batch = x.size(0);
  const int64_t in_features = x.size(1);
  ...
  queue.parallel_for(
      ...
      [=](sycl::nd_item<1> item) {
        ...
        for (int64_t index = item.get_local_id(0); index < in_features;
          ...
          const uint8_t *packed =
              weight_ptr + (row * blocks_per_row + block) * kBlockBytes;
          const int64_t half = in_block / 128;
          const int64_t group32 = (in_block % 128) / 32;
          const int64_t lane = in_block % 32;
          const int64_t low_offset = half * 64 + (group32 % 2) * 32 + lane;
          const int low_shift = group32 >= 2 ? 4 : 0;
          const int low = (packed[low_offset] >> low_shift) & 0x0F;
          const int high =
              (packed[128 + half * 32 + lane] >> (group32 * 2)) & 0x03;
          const int quant = (low | (high << 4)) - 32;
          const auto *scales = reinterpret_cast<const int8_t *>(packed + 192);
          const int scale = scales[half * 8 + group32 * 2 + lane / 16];
          const float d = static_cast<float>(
              *reinterpret_cast<const sycl::half *>(packed + 208));
          const float weight = d * static_cast<float>(scale * quant);
          for (int64_t tile = 0; tile < kTokenTile; ++tile) {
            const int64_t token = token_base + tile;
            if (token < batch) {
...
torch::Tensor q6_k_matvec(torch::Tensor x, torch::Tensor qweight) {
  TORCH_CHECK(x.device().is_xpu(), "x must be an XPU tensor");
  TORCH_CHECK(qweight.device() == x.device(),
              "qweight must be on the same XPU device as x");
  TORCH_CHECK(x.dim() == 2 && qweight.dim() == 2,
  ...
  TORCH_CHECK(qweight.scalar_type() == torch::kUInt8,
              "qweight must use uint8 storage");
  TORCH_CHECK(x.size(1) % 256 == 0,
              "Q6_K input width must be divisible by 256");
  TORCH_CHECK(qweight.size(1) == x.size(1) / 256 * 210,
              "qweight has invalid Q6_K row geometry");
  auto output = torch::empty({x.size(0), qweight.size(0)}, x.options());
  if (output.numel() == 0) {
    return output;
  }
  if (x.scalar_type() == torch::kFloat32) {
    launch_q6_k_matvec_tiled<float>(x, qweight, output);
  } else if (x.scalar_type() == torch::kBFloat16) {
    launch_q6_k_matvec_tiled<sycl::ext::oneapi::bfloat16>(x, qweight, output);
  } else {
    TORCH_CHECK(false, "q6_k_matvec supports float32 and bfloat16 inputs");
  }
  return output;
}
  ...
  queue.parallel_for(
      ...
      [=](sycl::nd_item<1> item) {
        ...
        for (int64_t index = item.get_local_id(0); index < in_features;
          ...
          const uint32_t aux =
              static_cast<uint32_t>(packed[66 + subblock * 4]) |
          ...
          const int sign_code =
              static_cast<int>(signs_ptr[(aux >> (pair * 7)) & 0x7F]);
...
template <typename scalar_t>
void launch_iq3_xxs_matvec_tiled(const torch::Tensor &x,
                                 const torch::Tensor &qweight,
                                 const torch::Tensor &table,
                                 const torch::Tensor &signs,
                                 torch::Tensor &output) {
...
torch::Tensor iq3_xxs_matvec(torch::Tensor x, torch::Tensor qweight,
  ...
  if (x.scalar_type() == torch::kFloat32) {
    if (x.size(0) >= 4) {
      launch_iq3_xxs_matvec_tiled<float>(x, qweight, table, signs, output);
    } else {
      launch_iq3_xxs_matvec<float>(x, qweight, table, signs, output);
    }
  } else if (x.scalar_type() == torch::kBFloat16) {
    if (x.size(0) >= 4) {
      launch_iq3_xxs_matvec_tiled<sycl::ext::oneapi::bfloat16>(
          x, qweight, table, signs, output);
    } else {
      launch_iq3_xxs_matvec<sycl::ext::oneapi::bfloat16>(x, qweight, table,
                                                         signs, output);
    }
  } else {
      ...
      [=](sycl::nd_item<1> item) {
        ...
        for (int64_t index = item.get_local_id(0); index < in_features;
          ...
          const int q_index =
              ...
              ((static_cast<int>(packed[66 + subblock]) << (8 - 2 * part)) &
               0x300);
          ...
          const int scale_nibble =
              scale_index % 2 == 0 ? scale_byte & 0x0F : scale_byte >> 4;
          const float d =
              static_cast<float>(*reinterpret_cast<const sycl::half *>(packed));
          const float scale =
              d * (0.5f + static_cast<float>(scale_nibble)) * 0.25f;
          const float sign = (sign_code & (1 << lane)) == 0 ? 1.0f : -1.0f;
          partial += static_cast<float>(x_ptr[token * in_features + index]) *
...
template <typename scalar_t>
void launch_iq2_s_matvec_tiled(const torch::Tensor &x,
                               const torch::Tensor &qweight,
                               const torch::Tensor &table,
                               torch::Tensor &output) {
  constexpr int64_t kBlockSize = 256;
      ...
      [=](sycl::nd_item<1> item) {
        ...
        for (int64_t index = item.get_local_id(0); index < in_features;
          ...
          const float scale =
              d * (0.5f + static_cast<float>(scale_nibble)) * 0.25f;
          const float sign = (sign_code & (1 << lane)) == 0 ? 1.0f : -1.0f;
          const float weight = scale * table_ptr[q_index * 8 + lane] * sign;
          for (int64_t tile = 0; tile < kTokenTile; ++tile) {
            const int64_t token = token_base + tile;
            if (token < batch) {
              partial[tile] +=
...
torch::Tensor iq2_s_matvec(torch::Tensor x, torch::Tensor qweight,
  ...
  TORCH_CHECK(table.numel() == 1024 * 8, "IQ2_S lookup table has invalid size");
  auto output = torch::empty({x.size(0), qweight.size(0)}, x.options());
  if (x.scalar_type() == torch::kFloat32) {
    if (x.size(0) >= 4) {
      launch_iq2_s_matvec_tiled<float>(x, qweight, table, output);
    } else {
      launch_iq2_s_matvec<float>(x, qweight, table, output);
    }
  } else if (x.scalar_type() == torch::kBFloat16) {
    if (x.size(0) >= 4) {
      launch_iq2_s_matvec_tiled<sycl::ext::oneapi::bfloat16>(x, qweight, table,
                                                             output);
    } else {
      launch_iq2_s_matvec<sycl::ext::oneapi::bfloat16>(x, qweight, table,
                                                       output);
    }
  } else {
  ...
  queue.parallel_for(
      ...
      [=](sycl::nd_item<1> item) {
        ...
        for (int64_t index = item.get_local_id(0); index < in_features;
          ...
          const int q_index =
              packed[2 + subblock * 8 + part * 2 + second] |
          ...
          const int scale_nibble =
              subblock % 2 == 0 ? scale_byte & 0x0F : scale_byte >> 4;
          const float d =
              static_cast<float>(*reinterpret_cast<const sycl::half *>(packed));
          const float scale =
              d * (1.0f + 2.0f * static_cast<float>(scale_nibble));
          const float sign =
              (sign_code & (1 << within_part)) == 0 ? 1.0f : -1.0f;
          partial += static_cast<float>(x_ptr[token * in_features + index]) *
                     (scale * table_ptr[q_index * 4 + grid_lane] * sign);
        }
        const float sum = sycl::reduce_over_group(item.get_group(), partial,
...
template <typename scalar_t>
void launch_iq3_s_matvec_tiled(const torch::Tensor &x,
                               const torch::Tensor &qweight,
                               const torch::Tensor &table,
                               torch::Tensor &output) {
  constexpr int64_t kBlockSize = 256;
...
torch::Tensor iq3_s_matvec(torch::Tensor x, torch::Tensor qweight,
  ...
  TORCH_CHECK(table.numel() == 512 * 4, "IQ3_S lookup table has invalid size");
  auto output = torch::empty({x.size(0), qweight.size(0)}, x.options());
  if (x.scalar_type() == torch::kFloat32) {
    if (x.size(0) >= 4) {
      launch_iq3_s_matvec_tiled<float>(x, qweight, table, output);
    } else {
      launch_iq3_s_matvec<float>(x, qweight, table, output);
    }
  } else if (x.scalar_type() == torch::kBFloat16) {
    if (x.size(0) >= 4) {
      launch_iq3_s_matvec_tiled<sycl::ext::oneapi::bfloat16>(x, qweight, table,
                                                             output);
    } else {
      launch_iq3_s_matvec<sycl::ext::oneapi::bfloat16>(x, qweight, table,
                                                       output);
    }
  } else {
...
template <typename scalar_t>
void launch_iq2_xxs_matvec(const torch::Tensor &x, const torch::Tensor &qweight,
                           const torch::Tensor &table,
                           const torch::Tensor &signs, torch::Tensor &output) {
  constexpr int64_t kBlockSize = 256;
      ...
      [=](sycl::nd_item<1> item) {
        ...
        for (int64_t index = item.get_local_id(0); index < in_features;
          ...
          const uint32_t low_word =
              static_cast<uint32_t>(packed[2 + subblock * 8]) |
          ...
          const uint32_t high_word =
              static_cast<uint32_t>(packed[6 + subblock * 8]) |
          ...
          const int q_index =
              static_cast<int>((low_word >> (8 * grid_index)) & 0xFF);
          const int sign_index =
              static_cast<int>((high_word >> (7 * grid_index)) & 0x7F);
          const int sign_code = static_cast<int>(signs_ptr[sign_index]);
          const float d =
              static_cast<float>(*reinterpret_cast<const sycl::half *>(packed));
          const float scale =
              d * (0.5f + static_cast<float>(high_word >> 28)) * 0.25f;
          const float sign = (sign_code & (1 << lane)) == 0 ? 1.0f : -1.0f;
          partial += static_cast<float>(x_ptr[token * in_features + index]) *
                     (scale * table_ptr[q_index * 8 + lane] * sign);
        }
...
template <typename scalar_t>
void launch_iq2_xxs_matvec_tiled(const torch::Tensor &x,
                                 const torch::Tensor &qweight,
                                 const torch::Tensor &table,
                                 const torch::Tensor &signs,
                                 torch::Tensor &output) {
...
torch::Tensor iq2_xxs_matvec(torch::Tensor x, torch::Tensor qweight,
  ...
  if (x.scalar_type() == torch::kFloat32) {
    if (x.size(0) >= 4) {
      launch_iq2_xxs_matvec_tiled<float>(x, qweight, table, signs, output);
    } else {
      launch_iq2_xxs_matvec<float>(x, qweight, table, signs, output);
    }
  } else if (x.scalar_type() == torch::kBFloat16) {
    if (x.size(0) >= 4) {
      launch_iq2_xxs_matvec_tiled<sycl::ext::oneapi::bfloat16>(
          x, qweight, table, signs, output);
    } else {
...
template <typename scalar_t>
void launch_iq2_xs_matvec(const torch::Tensor &x, const torch::Tensor &qweight,
                          const torch::Tensor &table,
                          const torch::Tensor &signs, torch::Tensor &output) {
  constexpr int64_t kBlockSize = 256;
...
template <typename scalar_t>
void launch_iq2_xs_matvec_tiled(const torch::Tensor &x,
                                const torch::Tensor &qweight,
                                const torch::Tensor &table,
                                const torch::Tensor &signs,
                                torch::Tensor &output) {
...
torch::Tensor iq2_xs_matvec(torch::Tensor x, torch::Tensor qweight,
  ...
  if (x.scalar_type() == torch::kFloat32) {
    if (x.size(0) >= 4) {
      launch_iq2_xs_matvec_tiled<float>(x, qweight, table, signs, output);
    } else {
      launch_iq2_xs_matvec<float>(x, qweight, table, signs, output);
    }
  } else if (x.scalar_type() == torch::kBFloat16) {
    if (x.size(0) >= 4) {
      launch_iq2_xs_matvec_tiled<sycl::ext::oneapi::bfloat16>(x, qweight, table,
                                                              signs, output);
    } else {
...
template <typename scalar_t>
void launch_iq4_xs_matvec(const torch::Tensor &x, const torch::Tensor &qweight,
                          torch::Tensor &output) {
  constexpr int64_t kBlockSize = 256;
...
template <typename scalar_t>
void launch_iq4_xs_matvec_tiled(const torch::Tensor &x,
                                const torch::Tensor &qweight,
                                torch::Tensor &output) {
  constexpr int64_t kBlockSize = 256;
  constexpr int64_t kBlockBytes = 136;
  ...
  queue.parallel_for(
      ...
      [=](sycl::nd_item<1> item) {
        ...
        for (int64_t index = item.get_local_id(0); index < in_features;
          ...
          const float d =
              static_cast<float>(*reinterpret_cast<const sycl::half *>(packed));
          const float weight = d * static_cast<float>(scale) * codebook[quant];
          for (int64_t tile = 0; tile < kTokenTile; ++tile) {
            const int64_t token = token_base + tile;
            if (token < batch) {
              partial[tile] +=
...
torch::Tensor iq4_xs_matvec(torch::Tensor x, torch::Tensor qweight) {
  ...
  if (x.scalar_type() == torch::kFloat32) {
    if (x.size(0) >= 4) {
      launch_iq4_xs_matvec_tiled<float>(x, qweight, output);
    } else {
      launch_iq4_xs_matvec<float>(x, qweight, output);
    }
  } else if (x.scalar_type() == torch::kBFloat16) {
    if (x.size(0) >= 4) {
      launch_iq4_xs_matvec_tiled<sycl::ext::oneapi::bfloat16>(x, qweight,
                                                              output);
    } else {
...
template <typename scalar_t>
void launch_iq1_s_matvec(const torch::Tensor &x, const torch::Tensor &qweight,
                         const torch::Tensor &table, torch::Tensor &output) {
  constexpr int64_t kBlockSize = 256;
...
template <typename scalar_t>
void launch_iq1_m_matvec(const torch::Tensor &x, const torch::Tensor &qweight,
                         const torch::Tensor &table, torch::Tensor &output) {
  constexpr int64_t kBlockSize = 256;
  ...
  queue.parallel_for(
      ...
      [=](sycl::nd_item<1> item) {
        ...
        for (int64_t index = item.get_local_id(0); index < in_features;
          ...
          const uint16_t scale_word =
              static_cast<uint16_t>(packed[scale_word_byte]) |
          ...
          const uint16_t d_bits =
              ((static_cast<uint16_t>(packed[48]) |
                (static_cast<uint16_t>(packed[49]) << 8)) >>
               12) |
              ((((static_cast<uint16_t>(packed[50]) |
                  (static_cast<uint16_t>(packed[51]) << 8)) >>
                 8) &
                0x00F0)) |
              ((((static_cast<uint16_t>(packed[52]) |
                  (static_cast<uint16_t>(packed[53]) << 8)) >>
                 4) &
                0x0F00)) |
              ((static_cast<uint16_t>(packed[54]) |
                (static_cast<uint16_t>(packed[55]) << 8)) &
               0xF000);
          const float d = static_cast<float>(
              *reinterpret_cast<const sycl::half *>(&d_bits));
          const float scale =
              d * (2.0f * static_cast<float>(scale_value) + 1.0f);
...
template <typename scalar_t>
void launch_iq1_m_matvec_tiled(const torch::Tensor &x,
                               const torch::Tensor &qweight,
                               const torch::Tensor &table,
                               torch::Tensor &output) {
  constexpr int64_t kBlockSize = 256;
...
torch::Tensor iq1_m_matvec(torch::Tensor x, torch::Tensor qweight,
  ...
  TORCH_CHECK(table.numel() == 2048 * 8, "IQ1_M lookup table has invalid size");
  auto output = torch::empty({x.size(0), qweight.size(0)}, x.options());
  if (x.scalar_type() == torch::kFloat32) {
    if (x.size(0) >= 4) {
      launch_iq1_m_matvec_tiled<float>(x, qweight, table, output);
    } else {
      launch_iq1_m_matvec<float>(x, qweight, table, output);
    }
  } else if (x.scalar_type() == torch::kBFloat16) {
    if (x.size(0) >= 4) {
      launch_iq1_m_matvec_tiled<sycl::ext::oneapi::bfloat16>(x, qweight, table,
                                                             output);
    } else {
      launch_iq1_m_matvec<sycl::ext::oneapi::bfloat16>(x, qweight, table,
                                                       output);
    }
  } else {
...
PYBIND11_MODULE(TORCH_EXTENSION_NAME, module) {
  ...
  module.def("q4_1_matvec", &q4_1_matvec, "SYCL Q4_1 matrix-vector product");
  module.def("iq4_nl_matvec", &iq4_nl_matvec,
             "SYCL IQ4_NL matrix-vector product");
  module.def("q5_0_matvec", &q5_0_matvec, "SYCL Q5_0 matrix-vector product");
  module.def("q5_1_matvec", &q5_1_matvec, "SYCL Q5_1 matrix-vector product");
  ...
  module.def("pq2_0_matvec", &pq2_0_matvec, "SYCL PQ2_0 matrix-vector product");
  ...
  module.def("q6_k_matvec", &q6_k_matvec, "SYCL Q6_K matrix-vector product");
  ...
  module.def("iq2_xs_matvec", &iq2_xs_matvec,
             "SYCL IQ2_XS matrix-vector product");
  module.def("iq4_xs_matvec", &iq4_xs_matvec,
             "SYCL IQ4_XS matrix-vector product");
