...
def hadamard_transform_sycl(
    x: torch.Tensor, signs: torch.Tensor, block_size: int, *, inverse: bool = False
) -> torch.Tensor:
    try:
    ...
    return _sycl_kernels.hadamard_transform(x, signs, block_size, inverse)
...
def iq4_nl_matvec_sycl(x: torch.Tensor, qweight: torch.Tensor) -> torch.Tensor:
    try:
        from freetoken.kernel import _sycl_kernels
    except ModuleNotFoundError as error:
        raise RuntimeError(
    ...
    return _sycl_kernels.iq4_nl_matvec(x, qweight)
...
def q5_0_matvec_sycl(x: torch.Tensor, qweight: torch.Tensor) -> torch.Tensor:
    ...
    return _sycl_kernels.q5_0_matvec(x, qweight)


def q5_1_matvec_sycl(x: torch.Tensor, qweight: torch.Tensor) -> torch.Tensor:
    try:
        from freetoken.kernel import _sycl_kernels
    except ModuleNotFoundError as error:
        raise RuntimeError(
    ...
    return _sycl_kernels.q5_1_matvec(x, qweight)
